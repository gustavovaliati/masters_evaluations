...
#python3 eval.py --model yolov2 --prpath ~/workspace/darkflow/jsonoutput/  --gtpath ~/workspace/datasets/pti/PTI01-bbox-labels/
...
def main():
    ...
    parser.add_argument(
        '--metric', choices=('voc_detection', 'all'),
        default='all')
    ...
    if args.model == 'yolov2':
        if not args.gtpath or not args.prpath:
            parser.error('Missing --gtpath or --prpath that are mandatory for yolov2.')
            return
...
def run_yolov2(args):
    flow_eval = DarkflowEvalPTI01(args.gtpath, args.prpath, args.impath, metric=args.metric)
    flow_eval.eval()

def run_chainercv(args):
    chainercv_eval = ChainercvEvalPTI01(
        args.gtpath, args.prpath, args.impath, args.pretrained_model,
